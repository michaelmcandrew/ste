...
<fieldset>
    ...
    <div id="event-fees">
	    ...
	    <dd>{$form.contribution_type_id.html}</dd>
            <dt>&nbsp;</dt>
	    <dd class="description">{ts}This contribution type will be assigned to payments made by participants when they register online.{/ts}</dd>
            <dt>{$form.fee_label.label}<span class="marker"> *</span> {if $action == 2}{include file='CRM/Core/I18n/Dialog.tpl' table='civicrm_event' field='fee_label' id=$id}{/if}</dt>
	    <dd>{$form.fee_label.html}</dd>
            <dt>&nbsp;</dt>
	    <dd class="description">{ts}This label is displayed with the list of event fees.{/ts}</dd>
            </dl>
        </div>
        ...
        <div id="payLater">
	     ...
	     <dd>{$form.is_pay_later.html}&nbsp;{$form.is_pay_later.label}<br />
        ...
        <div id="payLaterOptions">
          <dl>
             <dt>{$form.pay_later_text.label}<span class="marker"> *</span> {if $action == 2}{include file='CRM/Core/I18n/Dialog.tpl' table='civicrm_event' field='pay_later_text' id=$id}{/if}</dt>
	     <dd>{$form.pay_later_text.html|crmReplace:class:big}</dd>
             <dt>&nbsp;</dt>
	     <dd class="description">{ts}Text displayed next to the checkbox for the 'pay later' option on the contribution form.{/ts}</dd>
             <dt>{$form.pay_later_receipt.label}<span class="marker"> *</span> {if $action == 2}{include file='CRM/Core/I18n/Dialog.tpl' table='civicrm_event' field='pay_later_receipt' id=$id}{/if}</dt>
	     <dd>{$form.pay_later_receipt.html|crmReplace:class:big}</dd>
             <dt>&nbsp;</dt>
	     <dd class="description">{ts}Instructions added to Confirmation and Thank-you pages when the user selects the 'pay later' option (e.g. 'Mail your check to ... within 3 business days.').{/ts}
	     </dd>
          </dl>
        </div>
	    ...
	    <dd>{if $price eq false}
	    	...
	    	{else}
		    {$form.price_set_id.html}</dd>
		    <dt>&nbsp;</dt>
		    <dd class="description">{ts 1=$adminPriceSets}Select a pre-configured Price Set to offer multiple individually priced options for event registrants. Otherwise, select &quot;-none-&quot; and enter one or more fee levels in the table below. Create or edit Price Sets <a href='%1'>here</a>.{/ts}
	    	{/if}
	    </dd>
            </dl>
        </div>
    ...
    <div id="isDiscount">
         <dl>
         <dt class="extra-long-fourty">&nbsp;</dt>
	 <dd>{$form.is_discount.html}&nbsp;{$form.is_discount.label}<br /><span class="description">{ts}Check this box if you want to offer discounted fees based on registration date (e.g. 'early-registration discounts').{/ts}</span></dd>
         </dl>
    </div>
